...
public interface HCISUPCMS extends Library {
    public static HCISUPCMS hcISUPCMS = CreateSDKInstance();
     ...
     * @return
     */
    static HCISUPCMS CreateSDKInstance() {
        try {
            if (hcISUPCMS == null) {
                String path = CommonUtil.getLibPath();
                synchronized (HCISUPCMS.class) {
                    ...
                    try {
                        ...
                        if (osSelect.isWindows()) {
                            //win系统加载库路径(路径不要带中文)
                            strDllPath = path + "HCISUPCMS.dll";
                        } else if (osSelect.isLinux())
                            //Linux系统加载库路径(路径不要带中文)
                            strDllPath = System.getProperty("user.dir") + "/lib/libHCISUPCMS.so";
                        return (HCISUPCMS) Native.loadLibrary(strDllPath, HCISUPCMS.class);
                    } catch (Exception ex) {
            ...
            System.out.println("加载CMS SDK 失败");
        }
        return null;
    }
