...
public interface HCNetSDK extends Library {
     ...
     */
    static HCNetSDK createSDKInstance() {
        try {
...
//            path = path.replace("/target/", "/");
            ...
            if (INSTANCE == null) {
                if (osSelect.isWindows()) {
                    ...
                    path = path.replace("/", "\\");
                    //win系统加载库路径
                    return (HCNetSDK) Native.loadLibrary(path+"HCNetSDK.dll", HCNetSDK.class);
                }else {
                    return (HCNetSDK) Native.loadLibrary(path + "libhcnetsdk.so", HCNetSDK.class);
                }
            }
