...
public interface HCISUPSMS extends Library {
    public static HCISUPSMS hcISUPSMS = CreateSDKInstance();
     ...
     * @return
     */
    static HCISUPSMS CreateSDKInstance() {
        if (hcISUPSMS == null) {
            synchronized (HCISUPSMS.class) {
                ...
                try {
                    if (osSelect.isWindows())
                        //win系统加载库路径
                        strDllPath = CommonUtil.getLibPath()+"HCISUPStream.dll";
                    else if (osSelect.isLinux())
                        //Linux系统加载库路径
                        strDllPath = System.getProperty("user.dir") + "/lib/libHCISUPStream.so";
                    return  (HCISUPSMS) Native.loadLibrary(strDllPath, HCISUPSMS.class);
                } catch (Exception ex) {
